...
<div class="container-fluid">
    ...
    <form action="{{ route('admin.libros_virtuales.update', $libro->id_libro_virtual) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" name="resumen_sinopsis" id="resumen_sinopsis" rows="3">{{ $libro->resumen_sinopsis }}</textarea>
        </div>

        <div class="mb-3">
            <label for="url_archivo" class="form-label">Archivo PDF</label>
            <input type="file" class="form-control" name="url_archivo" id="url_archivo">
            @if($libro->url_archivo)
                <a href="{{ asset('storage/' . $libro->url_archivo) }}" class="btn btn-info mt-2" target="_blank">Ver Archivo Actual</a>
            @endif
        </div>

        <div class="mb-3">
            <label for="peso_archivo" class="form-label">Peso Archivo</label>
            <input type="text" class="form-control" name="peso_archivo" id="peso_archivo" value="{{ $libro->peso_archivo }}">
        </div>
        ...
        <button type="submit" class="btn btn-success"><i class="fas fa-save"></i> Actualizar Libro Virtual</button>
        <a href="{{ route('admin.libros_virtuales.index') }}" class="btn btn-secondary">Cancelar</a>
    </form>
</div>
<br><br>
@endsection

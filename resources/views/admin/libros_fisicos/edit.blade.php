...
@section('title', 'Editar Libro Físico')

@section('content')
<div class="container-fluid">
    <h1 class="mb-4"><i class="fas fa-edit"></i> Editar Libro Físico</h1>

    <form action="{{ route('admin.libros_fisicos.update', $libro->id_libro_fisico) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <input type="number" class="form-control" name="año_edicion" id="año_edicion" value="{{ $libro->año_edicion }}">
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="tipo_encuadernacion" id="tipo_encuadernacion" class="form-control">
                <option value="TAPA_DURA" @if($libro->tipo_encuadernacion=='TAPA_DURA') selected @endif>Tapa Dura</option>
                <option value="RUSTICA" @if($libro->tipo_encuadernacion=='RUSTICA') selected @endif>Rústica</option>
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="estado_promedio" id="estado_promedio" class="form-control">
                <option value="NUEVO" @if($libro->estado_promedio=='NUEVO') selected @endif>Nuevo</option>
                <option value="BUENO" @if($libro->estado_promedio=='BUENO') selected @endif>Bueno</option>
                <option value="REGULAR" @if($libro->estado_promedio=='REGULAR') selected @endif>Regular</option>
            </select>
        </div>

        <div class="mb-3">
            <label for="stock_total" class="form-label">Stock Total</label>
            <input type="number" class="form-control" name="stock_total" id="stock_total" value="{{ $libro->stock_total }}" required>
        </div>

        <div class="mb-3">
            <label for="stock_disponible" class="form-label">Stock Disponible</label>
            <input type="number" class="form-control" name="stock_disponible" id="stock_disponible" value="{{ $libro->stock_disponible }}" required>
        </div>

        <div class="mb-3">
            <label for="ubicacion_pasillo" class="form-label">Ubicación / Pasillo</label>
            <input type="text" class="form-control" name="ubicacion_pasillo" id="ubicacion_pasillo" value="{{ $libro->ubicacion_pasillo }}">
        </div>
        ...
        <div class="mb-3">
            ...
            @if($libro->imagen_portada)
                <img src="{{ asset('storage/' . $libro->imagen_portada) }}" alt="Portada" class="img-thumbnail mt-2" width="100">
            @endif
        </div>

        <div class="mb-3">
            <label for="palabras_clave" class="form-label">Palabras Clave</label>
            <textarea class="form-control" name="palabras_clave" id="palabras_clave" rows="2">{{ $libro->palabras_clave }}</textarea>
        </div>

        <button type="submit" class="btn btn-success"><i class="fas fa-save"></i> Actualizar Libro</button>
